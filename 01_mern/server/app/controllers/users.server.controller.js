...
exports.authenticate = function(req, res, next) {
	...
	if (!req.body.auth || !req.body.auth.username || !req.body.auth.password) {
		...
		return res.status(400).json({status:"error", message: "Missing username or password", data:null});
	}
...
exports.requiresLogin = function (req, res, next) {
    // Obtain the session token from the requests cookies,
	// which come with every request
	// console.log("requiresLogin req", req)
	const token = req.cookies.token
	console.log("requiresLogin", token)
	// if the cookie is not set, return an unauthorized error
	if (!token) {
	  return res.send({ screen: 'auth' }).end();
	}
